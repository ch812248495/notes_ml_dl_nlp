# Notes about ml, dl, npl, c++, go
## deep learning
- [批标准化( batch normalization)](./deep_learning/batch_normalization.md)

## c++
- [c++从编译到生成可执行文件流程](./c++/c++从编译到可执行文件流程.md)
- [const 关键字](./c++/const.md)
- [extern](./c++/extern.md)
- [new malloc 对比](./c++/new,malloc.md)
- [static](./c++/static.md)
- [volatile](./c++/volaile.md)
- [内存对齐](./c++/内存对齐.md)
- [内存模型](./c++/内存模型.md)
- [多态 虚函数 纯虚函数 虚函数表](./c++/多态,虚函数,纯虚函数,虚函数表.md)
- [引用和指针](./引用和指针.md)
- [c++11新特性](./c++/c++11.md)

## machine learning

## natural language processing
-  [对句向量编码方法的调研](./natural_language_processing/surveyOnSentenceEmbedding.md)
-  [inferSent方法和迁移学习](./natural_language_processing/inferSent.md)
-  [p-means 方法:一种广义的取平均方法](./natural_language_processing/p-mean.md)
-  [从 skip-thoughts 到 quick-thoughts:句向量中的 skip-gram 方法](./natural_language_processing/quick-thoughts.md)
-  [从多任务对话中学习句向量](./natural_language_processing/learnFromDialog.md)
-  [从语言模型中得到自适应的句向量编码](./natural_language_processing/ELMo.md)

## go
* [Introduction of go](./goLearning/go/README.md)
* [golang basic syntax](./goLearning/go/chapter1.md)
  * [Hello, go](./goLearning/go/basic-syntax-for-go.md)
  * [Foundation](./goLearning/go/foundation.md)
  * [function and flow control](./goLearning/go/function-and-flow-control.md)
  * [struct](./goLearning/go/struct.md)
  * [object-oriented](./goLearning/go/object-oriented.md)
  * [interface](./goLearning/go/interface.md)
  * [concurrence](./goLearning/go/concurrence.md)
* web server building
  * [How web works](./goLearning/web-foundation/how-web-works.md)
  * [Build a web server by Go](./goLearning/web-foundation/build-a-web-server-by-go.md)
  * [How Go makes it](./goLearning/web-foundation/how-go-makes-it.md)

## 计算机网络
- [http 协议](./computer_network/http协议.md)
- [TCP/IP](./computer_network/TCP_IP.md)
- [TCP详解](./computer_network/TCP详解.md)